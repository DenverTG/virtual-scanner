...
export class Effects {
  ...
  render(source: HTMLCanvasElement): void {
    ...
    this.runChain();
    const gl = this.gl;
    // The screen is far smaller than the output. Without mipmaps a 4 px
    // dither or a 1 px scan line aliases into large moiré blobs; with them
    // the preview averages down the way a print viewed from a distance does.
    gl.bindTexture(gl.TEXTURE_2D, this.full![0].tex);
  ...
  private uploadSource(source: HTMLCanvasElement): void {
    ...
    if (resized) {
      ...
      this.full = [this.makeTarget(this.srcW, this.srcH, gl.LINEAR, true), this.makeTarget(this.srcW, this.srcH, gl.LINEAR)];
    }
  }
  ...
  /** A render target; with `mips` it minifies through a mipmap chain (regenerated by the caller). */
  private makeTarget(w: number, h: number, filter: number, mips = false): Target {
    const gl = this.gl;
    const tex = this.makeTexture(filter);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, w, h, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
    if (mips) {
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR_MIPMAP_LINEAR);
      gl.generateMipmap(gl.TEXTURE_2D); // allocate every level so the texture is complete
    }
